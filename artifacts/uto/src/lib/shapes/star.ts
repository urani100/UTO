...
import { pointsToPathPx, smoothPathPx } from "../engine/path";
...
export const starMeta: ShapeMeta = {
  ...
  defaults: { points: 5, inner: 0.42, rotation: -90, outer: 220, smooth: 0 },
  params: [
    ...
    { key: "smooth", label: "Corners", min: 0, max: 1, step: 1, options: [
      { value: 0, label: "Sharp", aria: "Sharp vertices" },
      { value: 1, label: "Smooth", aria: "Smooth curves through vertices" },
    ]},
  ],
};
...
export function renderStar(state: CanvasState): ShapeRender {
  ...
  const useSmooth = (p.smooth ?? 0) >= 0.5;
  ...
  const d = useSmooth ? smoothPathPx(verts, true, 0.4) : pointsToPathPx(verts, true);
  return {
    guide: d,
    paths: [{ id: "star-path", d, fontScale: 1, opacity: 1, policy: { kind: "fit-once", underfill: 0.95 } }],
  };
}
